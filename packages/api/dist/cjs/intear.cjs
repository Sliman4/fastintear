  ...
  async connect() {
    try {
      ...
      this.ws.onopen = async () => {
        ...
        const appPublicKeyString = utils.publicKeyFromPrivate(this.appPrivateKey);
        const signatureBase58 = utils.signHash(messageBytes, this.appPrivateKey, { returnBase58: true });
      ...
      this.ws.onmessage = async (event) => {
        ...
        if ("Success" in message) {
          ...
          const signatureBytes = utils.fromBase58(sigData);
          ...
          publicKeyBytes = utils.fromBase58(base58PublicKey);
          const isValid = ed25519.ed25519.verify(signatureBytes, verifyMessageBytes, publicKeyBytes);
          if (!isValid) {
...
async function generateAuthSignature(privateKey, data, nonce) {
  ...
  const hashBytes = sha2.sha256(messageBytes);
  const signatureBase58 = utils.signHash(hashBytes, privateKey, { returnBase58: true });
...
function verifyLogoutSignature(logoutInfo, accountId, appPublicKeyString, userLogoutPublicKey) {
  try {
    ...
    const sigData = sigParts[1];
    const signatureToVerify = utils.fromBase58(sigData);
    let verifyKey;
    if (logoutInfo.caused_by === "User") {
    ...
    const publicKeyBytes = utils.fromBase58(base58PublicKey);
    return ed25519.ed25519.verify(
      signatureToVerify,
...
async function verifySessionStatus(savedData, logoutBridgeService, onStateUpdate) {
  try {
    ...
    const appPublicKeyString = utils.publicKeyFromPrivate(appPrivateKey);
    ...
    const signatureBase58 = utils.signHash(messageBytes, appPrivateKey, { returnBase58: true });
    const signatureString = `ed25519:${signatureBase58}`;
...
class WalletAdapter {
  ...
  async signIn({
    ...
    const privateKey = utils.privateKeyFromRandom();
      ...
      const listener = /* @__PURE__ */ __name(async (event) => {
        ...
        switch (event.data.type) {
          case "ready": {
            ...
            const publicKey = utils.publicKeyFromPrivate(privateKey);
            iframe.contentWindow?.postMessage(
              {
                type: "signIn",
                data: {
          ...
          case "connected": {
            ...
            result = {
              ...
              publicKey: utils.publicKeyFromPrivate(dataToSave.key)
            };
            iframe.contentWindow?.postMessage(
              {
                type: "close"
  ...
  async signOut() {
    ...
    if (savedData) {
      try {
        ...
        const messageBytes = new TextEncoder().encode(messageText);
        const hashBytes = sha2.sha256(messageBytes);
        const signatureBase58 = utils.signHash(hashBytes, appPrivateKey, { returnBase58: true });
        const signatureString = `ed25519:${signatureBase58}`;
  ...
  getState() {
    ...
    if (savedData) {
      return {
        ...
        publicKey: utils.publicKeyFromPrivate(savedData.key)
      };
    }
...
exports.default = intear_default;
//# sourceMappingURL=intear.cjs.map
//# sourceMappingURL=intear.cjs.map